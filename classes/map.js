class Map extends MagicCircle{
	...
	doChart(){
		...
		let jsonObject = config_namespace.JSON_OBJECT;
	...
	d3.select('#tipBox').remove();
		...
		var allStates=commonfunctions_namespace.getAllStates();
		var counter=0;
										...
										return d.color;
												...
												function getAllCrimesState(allStates,year,crimeType,data){
													...
													for(let i=0;i<allStates.length;i++){
														...
														let currentCrimeValue=commonfunctions_namespace.getCrimesAndDataByYearAndState(year, allStates[i], data);

														if(config_namespace.CONSTANTS.crimeCategories.Crimes.violentCrime.includes(crimeType)){
														...
														else if (config_namespace.CONSTANTS.crimeCategories.Crimes.propertyCrime.includes(crimeType)) {
